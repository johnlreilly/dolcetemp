...
;;;
var HeroUI=HeroUI||{heroFull:null,heroHalf:null,heroList:null,windowW:$(window).width(),windowH:$(window).height(),initialize:function(){HeroUI.initHeroSlides();HeroUI.animateSlides();},animateSlides:function(){$('.hero.cycle-slideshow').on('cycle-before',function(e,optionHash,outgoingSlideEl,incomingSlideEl,forwardFlag){$('.cycle-overlay').css('right','-1000px');$('.cycle-overlay').animate({right:"+=1000"},1000,function(){});});},goToLink:function(){if($('.hero').length>0){$('.hero img').each(function(){$(this).bind('click',function(){var hrefAttr=$(this).attr('data-href');if(typeof hrefAttr!==typeof undefined&&hrefAttr!==false){window.location.href=hrefAttr;}});});}
$('.cycle-overlay').on('click',function(){var hrefAttr=$('.cycle-slide-active').attr('data-href');window.location.href=hrefAttr;});},initHeroSlides:function(){$('.cycle-overlay').css({'right':'-1000','z-index':'1000'});$('.cycle-overlay').animate({right:"+=1000"},1000,function(){});HeroUI.goToLink();HeroUI.resizeHeroFrame();},resizeHeroFrame:function(){var heroObj=$('.slide-frame img'),heroObjH=heroObj.height(),aspectRatio=0.55,windowW=$(window).width();if(windowW>992){$('.slide-frame').css('height','500px');}},}
$(window).load(HeroUI.initialize);$(window).resize(HeroUI.resizeHeroFrame);
;;;